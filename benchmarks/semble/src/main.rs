...
impl Args {
    fn parse() -> Result<Self, String> {
        ...
        let mut parsed = Self {
            cases_dir: root.join("benchmarks/semble/cases"),
            ...
            json: root.join("benchmarks/semble/results/latest.json"),
            markdown: root.join("benchmarks/semble/results/latest.md"),
            codegraph_adapter: root.join("benchmarks/semble/codegraph_adapter.cjs"),
            codegraph: "codegraph".into(),
            quality_gates: root.join("benchmarks/semble/quality-gates.json"),
            sources: HashMap::new(),
            repetitions: 5,
            reset_indexes: true,
            check: false,
